...
// Virtual "antibodies" are rewarded for making it closer to the finish
...
void draw() {
 ...
 target.displayTarget();
  ...
  // Otherwise a new generation
   ...
   textAlign(RIGHT);
   fill(0);
   text("Generation #: " + population.getGenerations(),width-30,18);
   text("Lifetime remaining in cycles: " + ((lifetime-lifecycle)),width-30,36);
   text("Closest approach to target: " + population.shortestDist,width-30,54);
   text("Winning Rockets: " + population.winCount,width-30,72);
   text("Earliest Generation to Win: " + population.earliestGen,width-30,90);
   text("Fastest winning time in cycles: " + (population.earliestGen > 0 ? recordtime : 0),width-30,108);
