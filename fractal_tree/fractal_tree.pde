...
void setup() {
  ...
  // Create an empty ruleset
   ...
   tree = new Tree(lsys.getSentence(),width*2,TWO_PI/3);
   */
  ...
  /*Rule[] ruleset = new Rule[1];
   ...
   tree = new Tree(lsys.getSentence(),width-1,PI/2);
   */

  Rule[] ruleset = new Rule[1];
  ruleset[0] = new Rule('F', "FF+[+F-F-F]-[-F+F+F]");
  lsys = new LSystem("F", ruleset);
  tree = new Tree(lsys.getSentence(), height/3, radians(25));
}
