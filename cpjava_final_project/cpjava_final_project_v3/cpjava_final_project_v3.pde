...
void draw() {
  ...
  if (population.finished()) {
    ...
    println("Final phrase guess: "+population.getBest());
    noLoop();
  }
}
...
public void keyPressed() {                                                                                         //simulation controls
  ...
  if (running) noLoop();
    ...
    float timePassed = millis()/1000.0;
    println("Alphabot Evolution at Key Press took "+timePassed+" seconds");
    println(population.generations/timePassed   +" Alphabot generations were created per second.");
    println("Best phrase guess at Key Press: "+population.getBest());
    loop();
  }
}
